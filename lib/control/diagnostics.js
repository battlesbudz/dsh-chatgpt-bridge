...
export async function httpGetStatus(url, timeoutMs = 2000) {
    return new Promise((resolve) => {
        try {
            ...
            const req = getter({ host: u.hostname, port: u.port === '' ? undefined : Number(u.port), path: u.pathname + u.search, timeout: timeoutMs }, (res) => {
                res.resume();
                resolve(res.statusCode);
            });
            req.once('timeout', () => {
...
function probeDirect(u, headers, timeoutMs) {
    return new Promise((resolve) => {
        ...
        const req = (u.protocol === 'https:' ? httpsGet : httpGet)({ host: u.hostname, port: u.port === '' ? undefined : Number(u.port), path: u.pathname + u.search, headers, timeout: timeoutMs }, (res) => {
            res.resume();
