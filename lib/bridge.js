...
import { DEFAULT_WAIT_SECONDS, RequestIdMap, WAIT_POLL_MS, buildSupervisedGoalContext, clampWaitSeconds, executionView, fingerprintStart, isActiveStatus, isTerminalStatus, mapStartGoal, mapWaitGoal, titleFromGoal, } from './goal.js';
import { extractCommand, extractFilePath, foldGoalFacts, parseArgsJson, successfulKinds } from './goal-facts.js';
import { reconcileTodos } from './goal-reconcile.js';
import { buildGoalGraph, describeBlocked, detectDeferredKinds, inferBlockedKind, resolveStepRefs, } from './goal-graph.js';
...
import { GoalControlStore, appendGoalEvent, applyNativeGetGoalResult, applyRevision, createGoalRecord, fileStoreIo, goalControlDir, sliceHistory, supervisionGoal, } from './goal-control.js';
import { evaluateConstraint, findConstraintViolation, parseConstraints, parseExecutionMode, } from './goal-constraints.js';
...
function deferredKindsOf(record) {
    if (record === undefined)
        return [];
    return record.deferred_step_ids.filter((id) => KNOWN_KINDS.has(id));
}
function commandForCall(events, callId) {
...
function changedFileCountOf(events) {
    if (events === undefined)
        return 0;
    const seen = new Set();
    for (const event of events) {
...
function findPostHocViolation(facts, record) {
