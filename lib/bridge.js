...
export class Bridge {
    ...
    start() {
        ...
        this.ctx.on('approval/request', (request, next) => this.decideApproval(request, next));
        this.ctx.effect(() => () => {
            this.muxAbort?.abort();
    ...
    async decideApproval(request, next = () => Promise.resolve('rejected')) {
        ...
        if (workspacePath !== undefined && workspacePath !== '') {
            this.recordObservedExecutionFacts(request.agent.id, workspacePath, foldGoalFacts((request.agent.session?.events ?? [])));
    ...
    recordObservedExecutionFacts(sessionId, workspacePath, facts) {
        ...
        for (const tool of facts.tools) {
            ...
            if (this.idempotencyManager.check(fingerprint) !== null)
                continue;
